...
4. a canonical paymaster instance together with explicit paymaster balance reservation, if a paymaster frame is present, and
...
4. `pay` must execute in `VERIFY` mode, must target an account whose runtime code exactly matches the canonical paymaster implementation, and must successfully call `APPROVE(0x1)`.
...
We address this conflict with a safe, standardized paymaster contract. It is designed such that ether which enters it cannot leave except:

1. in the form of payment for a transaction, or
2. after a delay period.
