...
# Guarantor flag (bit 3 of flags) is only valid with VERIFY mode, and the frame
# must permit exactly APPROVE_PAYMENT as its approval scope.
for frame in tx.frames:
    if frame.flags & APPROVE_GUARANTOR_FLAG:
        assert frame.mode == VERIFY
        assert (frame.flags & APPROVE_SCOPE_MASK) == APPROVE_PAYMENT
```
...
A frame-level signature hash elides only the data of that frame, identified by index:
...
- If `APPROVE_EXECUTION` bit is set in scope:
  - Revert if `sender_approved` is already `true`.
  - Revert if `resolved_target != tx.sender`.
  - Set `sender_approved = true`.

- If `APPROVE_PAYMENT` bit is set in scope:
  - Revert if `resolved_target` has insufficient balance for `TXPARAM(0x06)`.
  - If the current frame has the `APPROVE_GUARANTOR_FLAG` set (guarantor mode):
    - Revert if `payer_approved` is already `true` — a guarantor cannot override a previous payer.
    - `sender_approved` is **not** required. See the [guarantor behavior](#guarantor-behavior) section below.
  - Otherwise (regular mode):
    - Revert if `sender_approved == false`.
    - If `payer_approved` is already `true`, refund `TXPARAM(0x06)` to the previous payer. This is a **payer override**.
  - Collect `TXPARAM(0x06)` from `resolved_target`.
  - Set `payer_approved = true` and record `resolved_target` as the payer.
...
| 0x09    | frameIndex   | `guarantor` (`(frame.flags >> 3) & 0x01`, returns 0/1)    |
...
1. Let `resolved_target = frame.target if frame.target is not null else tx.sender`, then execute a call with the specified `mode`, `flags`, `resolved_target`, `gas_limit`, `value`, and `data`.
   - If mode is `SENDER`:
       - If `sender_approved == false`:
           - If `payer_approved == true`, skip this frame.
...
2. If frame has mode `VERIFY` and the frame did not successfully call `APPROVE`:
   - If `payer_approved == true` (a prior guarantor frame has already approved payment), the frame's state changes are discarded and execution continues.
   - Otherwise, the transaction is invalid.

After executing all frames, verify that `payer_approved == true`. If it is not, the whole transaction is invalid. If it is:

1. If `sender_approved == true`, increment the sender's nonce.
2. Refund any unpaid gas to the gas payer.

#### Guarantor behavior

A **guarantor frame** is a `VERIFY` frame with the `APPROVE_GUARANTOR_FLAG` (bit 3 of `flags`) set. A guarantor unconditionally approves payment for the transaction: its successful `APPROVE(APPROVE_PAYMENT)` sets `payer_approved = true` without requiring `sender_approved`.

The effect on the rest of the transaction:

- Subsequent `VERIFY` frames (typically sender validation) that fail to call `APPROVE` do **not** invalidate the transaction. Their state changes are discarded and execution continues to the next frame.
- `SENDER` frames are skipped if `sender_approved == false`. They execute normally if `sender_approved == true`.
- The guarantor pays gas for the entire transaction regardless of whether sender validation later succeeds.
- If sender validation succeeds (`sender_approved == true`), the protocol increments the sender's nonce as usual. If sender validation fails, the sender's nonce is untouched and replay protection falls back to the guarantor's own mechanism — see the [canonical paymaster](#canonical-paymaster) section for the standardized approach.

A transaction MAY contain at most one guarantor frame. If multiple guarantor frames are present, only the first may successfully call `APPROVE`; any subsequent guarantor frame's `APPROVE(APPROVE_PAYMENT)` reverts because `payer_approved` is already set.
...
| `guarantee` | VERIFY | A guarantor frame (`APPROVE_GUARANTOR_FLAG` set) that unconditionally approves payment |
...
The **validation prefix** ends at the frame that sets `payer_approved = true`. The public mempool recognizes three base prefixes, each optionally preceded by a single `deploy` frame:
...
5. `guarantee` must execute in `VERIFY` mode with `APPROVE_GUARANTOR_FLAG` set and successfully call `APPROVE(APPROVE_PAYMENT)`. Its target's runtime code must exactly match the canonical paymaster implementation.
6. The sum of `gas_limit` values across the validation prefix must not exceed `MAX_VERIFY_GAS`.
7. Nodes should stop simulation immediately once `payer_approved = true` has been observed.

#### Canonical Paymaster Exception

The generic validation trace and opcode rules below apply to all frames in the validation prefix except a `pay` or `guarantee` frame whose target runtime code exactly matches the canonical paymaster implementation. The canonical paymaster implementation is explicitly designed to be safe for public mempool use and is therefore admitted by code match, successful `APPROVE(APPROVE_PAYMENT)`, and the paymaster accounting rules in this section, rather than by requiring it to satisfy each generic validation rule individually.
...
The canonical paymaster implementation has two code paths selected by the `APPROVE_GUARANTOR_FLAG` bit on the currently executing frame (readable via `FRAMEPARAM(0x09, TXPARAM(0x0B))`):

- **Paymaster mode** (`APPROVE_GUARANTOR_FLAG == 0`): the paymaster's signer authorizes the canonical signature hash `sig_hash = TXPARAM(0x08)`. Replay protection is provided by the sender's nonce: the regular `APPROVE_PAYMENT` path requires `sender_approved == true`, so each valid paymaster-mode transaction consumes exactly one sender nonce.

- **Guarantor mode** (`APPROVE_GUARANTOR_FLAG == 1`): `sender_approved` is *not* required for payment approval, so the sender nonce may or may not advance depending on whether sender validation succeeds. The canonical paymaster maintains a per-sender nonce mapping as fallback replay protection for the case where sender validation fails:
  ...
  **`guarantee` frame (VERIFY).** The frame's `frame.data` carries a `payer_nonce` alongside the signer's authorization. The paymaster:
  ...
  1. Reads the `bump_nonce` frame's shape via `FRAMEPARAM`/`FRAMEDATALOAD` at frame index `current + 2` (skipping the sender validation frame at `current + 1`) and requires:
     ...
     - `data` equals the ABI encoding of `bumpNonce(tx.sender, payer_nonce)`.
  2. Reads `guarantor_nonce[tx.sender]` via `SLOAD` and requires it to equal `payer_nonce`.
  3. Authenticates `guarantor_sig_hash = keccak(TXPARAM(0x09) || payer_nonce)` against the paymaster's signer. `TXPARAM(0x09)` is the frame-elide signature hash, which elides only the guarantor's own frame data while preserving every other frame's data (including the sender's `VERIFY` frame data). This binds the sender's signature to the guarantor's authorization, preventing an attacker from mutating the sender's `VERIFY` data to grief the guarantor.
  4. Calls `APPROVE(APPROVE_PAYMENT)`.
  ...
  **`bump_nonce` frame (DEFAULT).** The frame calls `paymaster.bumpNonce(sender, payer_nonce)`. The paymaster:
  ...
  2. Requires the `guarantee` frame at `current - 2` to be a successful self-targeted guarantor `VERIFY`, via `FRAMEPARAM`:
     ...
     - `FRAMEPARAM(0x03, current - 2) & APPROVE_GUARANTOR_FLAG != 0`,
  ...
  If sender validation fails and the `bump_nonce` frame reverts (e.g., out of gas), the guarantor nonce is not consumed and the same `guarantor_sig_hash` becomes replayable until the paymaster's balance is drained. This is a paymaster liveness concern, not a mempool-admission concern: mempool correctness and spam prevention do not depend on the nonce bump. The canonical paymaster enforces a minimum `gas_limit` on the `bump_nonce` frame in step (1) to make this impossible for a well-signed transaction; the concrete threshold is fixed by the canonical paymaster implementation and must cover a cold-slot `SSTORE`.
...
Sender validation in a frame transaction may use arbitrary EVM code, which in the general case cannot be safely executed by public mempool nodes: it may read disallowed state, use banned opcodes, or have mass-invalidation dependencies. Requiring sender validation to satisfy the mempool's generic trace rules effectively restricts senders to a narrow set of validation schemes (e.g., the default code's `ecrecover` or `P256VERIFY`), defeating a key goal of the abstraction.

The guarantor flag addresses this by letting a canonical paymaster commit to paying for a transaction unconditionally — even if sender validation subsequently fails. Mempool nodes simulate only the canonical guarantor's `VERIFY` frame (which, like a regular canonical paymaster, is admitted by code match) and do not need to understand or execute the sender's validation logic. Senders gain the freedom to implement arbitrary validation schemes while remaining eligible for public mempool propagation, at the cost of paying a guarantor.

The guarantor flag is encoded as a flag bit rather than a new `APPROVE` scope because the scope space in `flags` bits 0-1 is fully allocated (`APPROVE_SCOPE_MASK = 0x03`). Using a flag bit also communicates the correct model: the underlying `APPROVE` scope is still `APPROVE_PAYMENT`; the flag adjusts *when* payment can be approved (i.e., without requiring `sender_approved`). Because `flags` is covered by the canonical signature hash, the sender explicitly signs over the choice of whether the payer is a guarantor.

#### Sender nonce behavior in guarantor mode

The protocol increments the sender's nonce whenever `sender_approved == true` at the end of a valid transaction. This is a universal invariant that applies regardless of whether a guarantor is used. It is safe even though `payer_approved` is set without requiring `sender_approved`: if an attacker crafts a transaction naming a victim as `tx.sender` and pairs it with their own guarantor, the victim's validation frame will fail (the attacker cannot produce the victim's signature), `sender_approved` stays `false`, and the victim's nonce is untouched.

When sender validation fails, the sender's nonce is not incremented and replay protection falls back to the canonical paymaster's `guarantor_nonce[sender]`. The `bump_nonce` frame conditionally increments this mapping only when the preceding sender validation frame failed, avoiding unnecessary state writes in the success case.

#### Why a per-sender guarantor nonce?

When sender validation fails, the canonical paymaster must enforce its own replay protection via `guarantor_nonce`. A single paymaster-wide nonce would reintroduce a mass-invalidation vector: any failing guaranteed transaction would invalidate every other pending guaranteed transaction sharing the same paymaster-wide nonce. A per-sender nonce keyed by `tx.sender` (i.e., `guarantor_nonce[sender]`) localizes invalidation to a single (paymaster, sender) pair. Combined with the existing one-pending-tx-per-sender mempool rule, this reduces the cross-transaction dependency graph to the same shape as non-guaranteed transactions.

#### Authenticating the payer_nonce

The canonical signature hash (`TXPARAM(0x08)`) elides all `VERIFY` frame `data`, so placing the `payer_nonce` in the guarantor frame's data does not commit it to the tx-wide `sig_hash`. The canonical paymaster instead authenticates a separate `guarantor_sig_hash = keccak(frame_sig_hash || payer_nonce)` against the paymaster's signer key, where `frame_sig_hash = TXPARAM(0x09)` is the frame-elide signature hash that elides only the guarantor's own frame data. This matches the general rule stated in the [Canonical signature hash](#canonical-signature-hash) section: any verifier that depends on `VERIFY` frame data must authenticate that data independently.

#### Why the guarantor uses `TXPARAM(0x09)` instead of `TXPARAM(0x08)`

`TXPARAM(0x08)` elides *all* `VERIFY` frame data, including the sender's validation signature. This creates a griefing vector: an attacker can observe a pending guaranteed transaction, replace the sender's `VERIFY` frame data with garbage, and resubmit. The sender's validation fails, the guarantor still pays (by design), and the attacker has wasted the guarantor's gas without cost.

`TXPARAM(0x09)` closes this gap. It elides only the currently executing frame's data — in this case, the guarantor's own `VERIFY` frame — while preserving every other frame's data, including the sender's `VERIFY` frame data. The guarantor signs over `keccak(TXPARAM(0x09) || payer_nonce)`, which commits to the sender's exact signature bytes. Mutating the sender's `VERIFY` data changes the frame-elide hash, invalidating the guarantor's signature and making the transaction fail the guarantor's own validation — exactly the desired outcome.

This does not compromise the general design principle of elided `VERIFY` data. The *sender's* signature still cannot reference its own `VERIFY` data (the circularity problem that elision was designed to solve). `TXPARAM(0x09)` simply allows a *different* frame to bind `VERIFY` data that it does not own.

#### Why nonce management is split across two frames

`VERIFY` frames execute under `STATICCALL` semantics and cannot write to storage; the `APPROVE` opcode is the only protocol-defined exception. The canonical paymaster therefore cannot increment `guarantor_nonce[sender]` from within its `guarantee` frame. Rather than widening the `VERIFY` invariant or introducing a new frame mode, the design uses the same frame-introspection pattern demonstrated by Example 4 (EOA paying gas in ERC-20s): a `VERIFY` paymaster frame inspects a later frame via `FRAMEPARAM`/`FRAMEDATALOAD`, confirms it carries out the intended state change, and only then approves payment.

The `bump_nonce` frame is placed after the sender validation frame (at `current + 2` from the guarantee) so that it can read the sender validation frame's status via `FRAMEPARAM(0x05, current - 1)` and conditionally decide whether to bump the guarantor nonce. `DEFAULT` mode is required (rather than `SENDER`) because `sender_approved` may still be `false` at this point — a `SENDER` frame would be skipped under guarantor behavior.

Unlike Example 4's transfer frame, which is only reachable as part of a validated transaction, `bumpNonce` is a directly callable method on the paymaster contract. It therefore re-checks, via `FRAMEPARAM`, that the guarantee frame at `current - 2` is a successful self-targeted guarantor `VERIFY`, rejecting any attempt to invoke `bumpNonce` standalone or out of context. This prevents a third party from burning a victim sender's guarantor nonce by submitting an unrelated transaction that simply calls `bumpNonce(victim, ...)`.

### Payer override

A later `APPROVE_PAYMENT` (or `APPROVE_PAYMENT_AND_EXECUTION`) may override a previous payer. When this happens, the protocol refunds the transaction's maximum cost to the original payer and collects it from the new one. The payer recorded in the receipt is the final payer.

This enables a "guarantor as admission bond" pattern: a guarantor approves payment to get the transaction into the mempool, but the sender later takes over payment after performing an in-transaction action (e.g., swapping ERC-20 tokens to ETH). If the sender's takeover fails — for example because the swap did not yield enough ETH — the `VERIFY` frame reverts, its state changes (including the payer switch) are discarded, and the guarantor remains the payer. `VERIFY` frame atomicity provides the fallback for free.

A guarantor frame (`APPROVE_GUARANTOR_FLAG` set) cannot itself override a previous payer; only regular-mode `APPROVE_PAYMENT` and `APPROVE_PAYMENT_AND_EXECUTION` can. This prevents a second guarantor from silently taking over payment without sender consent. Because `sender_approved` is required for regular-mode payment approval, only a sender-authenticated frame can trigger a payer override.
...
| Frame | Caller      | Target        | Value | Flags                                    | Data                                  | Mode    |
| ----- | ----------- | ------------- | ----- | ---------------------------------------- | ------------------------------------- | ------- |
| 0     | ENTRY_POINT | Guarantor     | 0     | APPROVE_PAYMENT \| APPROVE_GUARANTOR_FLAG | payer_nonce, Guarantor sig            | VERIFY  |
...
- Frame 0 (`guarantee`): The canonical guarantor checks that frame 2 is a `DEFAULT` call to itself invoking `bumpNonce(tx.sender, payer_nonce)` with a `gas_limit` sufficient to cover a cold-slot `SSTORE`, verifies `guarantor_nonce[tx.sender] == payer_nonce`, authenticates its own signer over `keccak(TXPARAM(0x09) || payer_nonce)` (the frame-elide sig hash, which binds the sender's `VERIFY` data), and calls `APPROVE(APPROVE_PAYMENT)`. This sets `payer_approved = true` without requiring `sender_approved`.
