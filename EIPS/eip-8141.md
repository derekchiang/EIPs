...
|    3 | `SENDER_ATOMIC` | Execute frame as `sender` atomically with consecutive `SENDER_ATOMIC` frames |
...
Same as `SENDER` mode, except that when a `SENDER_ATOMIC` frame reverts, all consecutive `SENDER_ATOMIC` frames that immediately precede the frame are also reverted, and all consecutive `SENDER_ATOMIC` frames that immediately follow the frame are skipped from execution.
...
For `SENDER_ATOMIC` frames that are part of a reverted atomic block, all frames in the block share `status = 0`. Frames that executed before the reverting frame have their actual `gas_used`; frames that were skipped (those following the reverting frame) have `gas_used = 0`.
...
1. Execute a call with the specified `mode`, `target`, `gas_limit`, and `data`.
   ...
   - If mode is `SENDER` or `SENDER_ATOMIC`:
       - `sender_approved` and `payer_approved` must be `true`. If not, the transaction is invalid.
   ...
   - If a frame's execution reverts:
     - If the frame is `SENDER_ATOMIC`:
       - Revert all consecutive `SENDER_ATOMIC` frames that immediately precede this frame.
       - Skip all consecutive `SENDER_ATOMIC` frames that immediately follow this frame.
     - Otherwise:
       - Its state changes are discarded and execution proceeds to the next frame.
...
- If `mode` is `VERIFY`:
  ...
  - If `signature_type` is:
    ...
    - Otherwise revert.
  - Call `APPROVE(scope)`.
- If `mode` is not `VERIFY`, revert the frame.
...
- For the purposes of gas accounting of warm / cold state status, the journal of such touches is shared across frames. When a `SENDER_ATOMIC` frame reverts, its state changes are discarded but any storage slots it warmed remain warm for subsequent frames.
...
By default, frames are not atomic.  This is important because we don't want SENDER frames to revert VERIFY frames, which would revert gas payments.

One might wonder why we didn't make SENDER frames atomic with one another.  The reason is that there are use cases such as Example 2 where the paymaster (using a VERIFY frame) needs the guarantee that the following SENDER frame does not revert no matter what happens to the following SENDER frames, since the paymaster would lose money if ERC-20 tokens were not transferred to it.

However, atomic batching is still useful in many use cases (e.g. `approve + swap`), so we include a `SENDER_ATOMIC` mode so users can leverage atomicity where they need it.
...
### Example 4: Atomic Batching

This example shows two `SENDER_ATOMIC` blocks in a single transaction. A `SENDER` frame between them breaks the consecutive chain, creating two independent atomic groups.

| Frame | Caller      | Target         | Data      | Mode           |
| ----- | ----------- | -------------- | --------- | -------------- |
| 0     | ENTRY_POINT | Null (sender)  | Signature | VERIFY         |
| 1     | Sender      | Target 1       | Call data | SENDER_ATOMIC  |
| 2     | Sender      | Target 2       | Call data | SENDER_ATOMIC  |
| 3     | Sender      | Target 3       | Call data | SENDER         |
| 4     | Sender      | Target 4       | Call data | SENDER_ATOMIC  |
| 5     | Sender      | Target 5       | Call data | SENDER_ATOMIC  |

- Frame 0: Verifies signature and calls `APPROVE(0x2)`.
- Frames 1–2 (atomic block 1): If frame 2 reverts, frame 1 is also reverted. If frame 1 reverts, frame 2 is skipped.
- Frame 3: Executes independently; its outcome does not affect either atomic block.
- Frames 4–5 (atomic block 2): If frame 5 reverts, frame 4 is also reverted. If frame 4 reverts, frame 5 is skipped. The outcome of block 2 does not affect block 1.
