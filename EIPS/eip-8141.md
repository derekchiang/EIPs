...
The canonical paymaster implementation has two code paths, selected by the shape of the currently executing frame's `data` (the paymaster examines `msg.data.length` to decide):

- **Paymaster mode** (65-byte calldata: `r || s || v`): the paymaster's signer authorizes the canonical signature hash `sig_hash = TXPARAM(0x08)`. The paymaster calls `APPROVE(APPROVE_PAYMENT)`. Replay protection is provided by the sender's nonce: `APPROVE_PAYMENT` alone does not satisfy the transaction validity condition — `sender_approved` must also be set — so each valid paymaster-mode transaction consumes exactly one sender nonce.

- **Guarantor mode** (97-byte calldata: `r || s || v || payer_nonce`): the paymaster calls `APPROVE(APPROVE_GUARANTEE)`, which alone satisfies the validity condition. The sender nonce advances only if sender validation succeeds; otherwise, replay protection falls back to a per-sender nonce mapping in the paymaster's storage:
...
| Frame | Caller      | Target        | Value | Flags                                    | Data                                  | Mode    |
| ----- | ----------- | ------------- | ----- | ---------------------------------------- | ------------------------------------- | ------- |
| 0     | ENTRY_POINT | Guarantor     | 0     | APPROVE_GUARANTEE                        | payer_nonce, Guarantor sig            | VERIFY  |
...
- Frame 0 (`guarantee`): The canonical guarantor checks that frame 2 is a `DEFAULT` call to itself invoking `bumpNonce(tx.sender, payer_nonce)` with a `gas_limit` sufficient to cover a cold-slot `SSTORE`, verifies `guarantor_nonce[tx.sender] == payer_nonce`, authenticates its own signer over `keccak(TXPARAM(0x09) || payer_nonce)` (the frame-elide sig hash, which binds the sender's `VERIFY` data), and calls `APPROVE(APPROVE_GUARANTEE)`. This sets `guarantor_approved = true`, satisfying the transaction validity condition without requiring `sender_approved`.
